...
fn is_safe(report: []u8, recurred: bool) !bool {
    ...
    var first_failure = safe and !recurred;
    ...
    while ((safe or first_failure) and remaining.len > 0) {
    ...
    if (!safe and !recurred) {
        _, remaining = try parse_int(report);
        _, remaining = skip_until_digit(remaining);

        return is_safe(remaining, true);
    } else {
        return safe;
    }
}
